...
def upgrade() -> None:
    ...
    op.execute("""
        ALTER TABLE polls ALTER COLUMN channel_id DROP NOT NULL
    """)
    
    # Step 2: Create a new table for poll_type based leaderboards
    op.execute("""
        CREATE TABLE IF NOT EXISTS poll_type_leaderboards (
    ...
    op.execute("""
        CREATE INDEX IF NOT EXISTS idx_poll_type_leaderboards_guild_id ON poll_type_leaderboards (guild_id);
        CREATE INDEX IF NOT EXISTS idx_poll_type_leaderboards_poll_type ON poll_type_leaderboards (poll_type);
        CREATE INDEX IF NOT EXISTS idx_poll_type_leaderboards_user_id ON poll_type_leaderboards (user_id);
    """)
    
    # Step 4: Migrate data from channel_leaderboards to poll_type_leaderboards
    # We'll group by user_id and poll_type
    op.execute("""
        INSERT INTO poll_type_leaderboards (guild_id, poll_type, user_id, points, total_correct, rank, last_updated)
        SELECT 
            p.guild_id,
            p.poll_type,
            cl.user_id,
        ...
        FROM channel_leaderboards cl
        JOIN poll_messages pm ON cl.channel_id = pm.channel_id
    ...
    op.execute("""
        DROP TABLE IF EXISTS channel_leaderboards
    """)
...
def downgrade() -> None:
    ...
    op.execute("""
        UPDATE polls SET channel_id = 0 WHERE channel_id IS NULL;
        ALTER TABLE polls ALTER COLUMN channel_id SET NOT NULL
    """)
    
    # Step 3: Drop the new poll_type_leaderboards table
    op.execute("""
        DROP TABLE IF EXISTS poll_type_leaderboards
    """) 