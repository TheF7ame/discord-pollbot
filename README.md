...
## 13. Scaling Down or Shutting Down the Service

When you need to reduce costs or temporarily disable the bot, you have several options:

### Option 1: Set minimum instances to zero

This approach allows the service to scale down to zero instances when inactive, but remain available to scale up automatically when needed:

```bash
gcloud run services update discord-poll-bot \
  --region=us-central1 \
  --min-instances=0
```

**Note:** Discord bots maintain an active WebSocket connection, so scaling down may not be immediate. It may take 15-30 minutes for the instance to be considered "inactive" and shut down.

### Option 2: Redirect traffic to a "parked" revision

Create and deploy a minimal revision that doesn't actually run the bot:

```bash
# Create a minimal Dockerfile in a temporary directory
mkdir temp_shutdown && cd temp_shutdown
echo 'FROM alpine:latest
CMD ["sleep", "infinity"]' > Dockerfile

# Build and deploy this as a new revision
gcloud builds submit --tag gcr.io/YOUR_PROJECT_ID/discord-poll-parked
gcloud run deploy discord-poll-bot \
  --image gcr.io/YOUR_PROJECT_ID/discord-poll-parked \
  --region=us-central1 \
  --min-instances=0
```

### Option 3: Completely delete the service

Only use this if you want to permanently remove the service:

```bash
gcloud run services delete discord-poll-bot --region=us-central1
```

### Option 4: Force disconnect the Discord bot

To immediately disconnect the bot from Discord (while leaving the Cloud Run service intact):

1. Visit the [Discord Developer Portal](https://discord.com/developers/applications)
2. Select your bot application
3. Go to "Bot" section
4. Reset the token (this will invalidate the current connection)
5. Update the token in Google Secret Manager when you want to reconnect
